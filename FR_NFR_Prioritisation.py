...
class Agent_Prob:
    def __init__(self, N, NFR_Count, landscape, fr_focus=0.5):
        self.N = N + NFR_Count
        self.state = np.random.choice([0, 1], self.N).tolist()
        self.landscape = landscape
        self.fr_focus = fr_focus
    ...
    def search(self):
        ...
        step = 1 if np.random.uniform(0, 1) < self.fr_focus else -1
        if step == 1:
...
# Search process (Replace parameters with desired values)
N = 16
NFR_Count = 4
K = 15
problem_spaces = {}
        ...
        for i in range(landscape_repetitions):  # landscape repetitions
            ...
            for _ in range(agent_num):
                agent = Agent_Prob(
                    ...
                    landscape=landscape,
                    NFR_Count=problem_space_configs["NFR_Count"],
                    fr_focus=problem_space_configs["fr_focus"],
                )
                agents.append(agent)
            ...
            pbar.update(1)
        np.savetxt(
            f"prioritisation_N{N}K{K}_{problem_space_configs['fr_focus']}.csv",
            agents_performance,
            delimiter=",",
...
f = open(f"prioritisation_N{N}K{K}_results.json", "w")
f.write(json)
f.close()
