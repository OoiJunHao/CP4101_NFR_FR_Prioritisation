...
class LandScape_MultiGoal:
    ...
    def create_influence_matrix(self):
        ...
        self.FR_Range = [i for i in range(0, self.N)]
        self.NFR_Range = [i for i in range(self.N, self.N + self.NFR_Count)]
    ...
    def calculate_fitness(self, state):
        ...
        for i in range(len(state)):
            ...
            if i >= len(self.FR_Range):
                res_nfr += self.FC[i][index]
            else:
                res_fr += self.FC[i][index]
        return [res_fr / len(self.FR_Range), res_nfr / len(self.NFR_Range)]
    ...
    def initialize(self, first_time=True, norm=True):
        ...
        # normalization
        if norm:
            normalizor_fr = float(max([val[0] for val in self.cache.values()]))
            min_normalizor_fr = float(min([val[0] for val in self.cache.values()]))
            normalizor_nfr = max([val[1] for val in self.cache.values()])
...
f = open(f"multiGoal__N{N}__together__results.json", "w")
...
f = open(f"multiGoal__N{N}__separate__results.json", "w")
