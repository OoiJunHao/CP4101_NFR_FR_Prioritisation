...
with tqdm(
    ...
    for problem_space_name, problem_space_configs in problem_spaces.items():
        ...
        for i in range(landscape_repetitions):  # landscape repetitions
            ...
            pbar.update(1)
        np.savetxt(
            f"project_SizeComplex__N{N}__together.csv",
            agents_performance,
            delimiter=",",
...
f = open(f"project_SizeComplex__N{N}__together__results.json", "w")
...
with tqdm(
    ...
    for problem_space_name, problem_space_configs in problem_spaces.items():
        ...
        for i in range(landscape_repetitions):  # landscape repetitions
            ...
            pbar.update(1)
        np.savetxt(
            f"project_SizeComplex__N{N}__separate.csv",
            agents_performance,
            delimiter=",",
        )  # save to csv for analysis
...
f = open(f"project_SizeComplex__N{N}__separate__results.json", "w")
